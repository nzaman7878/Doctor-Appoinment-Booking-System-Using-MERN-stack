...
const DoctorAppointments = () => {
  ...
  return (
    <div className="w-full max-w-6xl m-5 mt-8 pb-10">
      ...
      >
        <div className='max-sm:hidden grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 py-4 px-6 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 font-semibold text-sm'>
          <p>#</p>
          <p>Patient</p>
          <p>Payment</p>
          <p>Age</p>
            ...
            {filteredAppointments.length > 0 ? (
              [...filteredAppointments].reverse().map((item, index) => (
                <motion.div 
                  ...
                  className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 items-center text-slate-600 dark:text-slate-400 py-4 px-6 hover:bg-slate-50/80 dark:hover:bg-slate-800/80 transition-colors duration-200' 
                  key={item._id}
                >
                  <p className='max-sm:hidden font-medium'>{index + 1}</p>

                  <div className='flex items-center gap-3'>
                    <img className='w-10 h-10 object-cover rounded-full shadow-sm border-2 border-white dark:border-slate-700' src={item.userData.image} alt="" />
                    <p className='font-medium text-slate-800 dark:text-slate-200'>{item.userData.name}</p>
                  </div>

                  <div>
                    <p className='text-xs font-semibold inline border border-primary px-3 py-1 rounded-full text-primary dark:border-indigo-400 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-400/10'>
